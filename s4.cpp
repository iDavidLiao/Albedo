...
//  VisionHWp4
//
//  Created by David Liao on 11/28/17.
...
main(int argc, char **argv){
    ...
    const string ithreshold(argv[5]);
    ...
    Image output_image;
    if (!ReadImage(output_file, &output_image)) {
    ...
    //calculate the albedo with the X Y and Z values
    //scale it by *128 to fit the pixel values
    for(int i = 0; i < an_image.num_rows(); i++)
    {
        for(int j = 0; j < an_image.num_columns(); j++)
        {
            if((an_image.GetPixel(i, j) > threshold) && (an_image2.GetPixel(i, j) > threshold) && an_image3.GetPixel(i, j) > threshold)
            {
                ...
                Albedo = sqrt((SurfaceNormalX*SurfaceNormalX)+(SurfaceNormalY*SurfaceNormalY)+(SurfaceNormalZ*SurfaceNormalZ));
                ...
                {
                    Albedo = 255;
                    output_image.SetPixel(i,j, Albedo);
                }
                else
                {
                    output_image.SetPixel(i,j, Albedo);
